# ==================================================
# SCANNER NAME:
# Demand and Supply Trading – MONTHLY
# ==================================================
...
ACCESS_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..toaCwLxiD7SYxgPAtM2Cos5R248xLF8aRCM2KFMak-o"
...
LEG_IN_SINGLE_MULTIPLIER  = 1.0
LEG_OUT_SINGLE_MULTIPLIER = 2.0

CONT_MIN_CANDLES = 2
CONT_MAX_CANDLES = 3
CONT_BODY_PCT    = 0.50
LEG_IN_CONT_MULTIPLIER  = 1.0
...
def run_scan():
    ...
    for s in symbols:
        print(f"🔍 Monthly scanning {s}")
        df = fetch_data(fyers_symbol(s))
        if df is None or df.empty:
            continue

        mdf = to_monthly(df)
        zones = scan_symbol(mdf, s)
        all_zones.extend(zones)

    if not all_zones:
        print("❌ No monthly zones found")
        return

    dfout = pd.DataFrame(all_zones)
    fname = f"monthly_zone_scan_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
    dfout.to_csv(os.path.join(OUTPUT_FOLDER, fname), index=False)

    print("\n✅ MONTHLY ZONE SCAN COMPLETE")
