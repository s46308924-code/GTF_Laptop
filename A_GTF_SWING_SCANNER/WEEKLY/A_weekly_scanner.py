# ==================================================
# SCANNER NAME:
# Demand and Supply Trading – WEEKLY
# ==================================================
...
LEG_IN_SINGLE_MULTIPLIER  = 2.0
LEG_OUT_SINGLE_MULTIPLIER = 3.0

CONT_MIN_CANDLES = 2
CONT_MAX_CANDLES = 3
CONT_BODY_PCT    = 0.50
LEG_IN_CONT_MULTIPLIER  = 2.5
LEG_OUT_CONT_MULTIPLIER = 3.5
...
def run_scan():
    ...
    for s in symbols:
        print(f"🔍 Weekly scanning {s}")
        df = fetch_data(fyers_symbol(s))
        if df is None or df.empty:
            continue

        wdf = to_weekly(df)
        zones = scan_symbol(wdf, s)
        all_zones.extend(zones)

    if not all_zones:
        print("❌ No weekly zones found")
        return

    dfout = pd.DataFrame(all_zones)
    fname = f"weekly_zone_scan_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
    dfout.to_csv(os.path.join(OUTPUT_FOLDER, fname), index=False)

    print("\n✅ WEEKLY ZONE SCAN COMPLETE")
