#
# SCANNER NAME:
# Demand and Supply Trading – QUARTERLY
...
DAILY_TF = "1D"
API_DAY_LIMIT = 365
HIST_YEARS = 10          # 5 saal = ~20 quarters — kafi data
...
def run_scan():
    ...
    for s in symbols:
        print(f"🔍 Quarterly scanning {s}")
        df = fetch_data(fyers_symbol(s))
        if df is None or df.empty:
            continue

        qdf = to_quarterly(df)
        zones = scan_symbol(qdf, s)
        all_zones.extend(zones)

    if not all_zones:
        print("❌ No quarterly zones found")
        return

    dfout = pd.DataFrame(all_zones)
    fname = f"quarterly_zone_scan_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
    dfout.to_csv(os.path.join(OUTPUT_FOLDER, fname), index=False)

    print("\n✅ QUARTERLY ZONE SCAN COMPLETE")
