# 
# DEMAND & SUPPLY TRADING – WEEKLY SCANNER 
...
def valid_continuous(df, start_idx, zone_range, multiplier):
    ...
    while (
        ...
        and len(candles) < WEEKLY_CONT_MAX
    ):
        if body_pct(df.iloc[i]) < WEEKLY_CONT_BODY_PCT:
            break
        candles.append(df.iloc[i])
        i -= 1

    if len(candles) < WEEKLY_CONT_MIN:
        return False
...
def scan_weekly_zones(df):
    ...
    while i >= 3:
        ...
        legin_ok = (
            candle_range(legin) >= zone_range * WEEKLY_LEG_IN_SINGLE_M or
            valid_continuous(df, j, zone_range, WEEKLY_LEG_IN_CONT_M)
        )

        legout_ok = (
            candle_range(legout) >= zone_range * WEEKLY_LEG_OUT_SINGLE_M or
            valid_continuous(df, i, zone_range, WEEKLY_LEG_OUT_CONT_M)
        )
...
def run():
    ...
    if results:
        ...
        print("✅ WEEKLY CSV CREATED:", fname)
    else:
