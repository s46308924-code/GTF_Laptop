#
# WEEKLY DEMAND & SUPPLY SCANNER
# (MONTHLY + QUARTERLY CSV BASED)
...
QUARTERLY_CSV_DIR = "output/quarterly"
...
QUARTERLY_AREA_MULTIPLIER = 0.5

# BUTTONS SETTING

WEEKLY_BASE_BODY_PCT = 0.50
WEEKLY_LEG_BODY_PCT  = 0.50
WEEKLY_MAX_BASE = 3

WEEKLY_LEG_IN_SINGLE_M  = 1.5
WEEKLY_LEG_OUT_SINGLE_M = 2.0
WEEKLY_CONT_MIN = 2
WEEKLY_CONT_MAX = 3
WEEKLY_CONT_BODY_PCT = 0.50
WEEKLY_LEG_IN_CONT_M  = 1.5
WEEKLY_LEG_OUT_CONT_M = 2.0
...
def scan_zones(df):
    ...
    while i >= 3:
        legout = df.iloc[i]
        if not is_leg(legout, WEEKLY_LEG_BODY_PCT):
            i -= 1
            continue

        base = []
        j = i - 1
        while j >= 0 and is_base(df.iloc[j], WEEKLY_BASE_BODY_PCT) and len(base) < WEEKLY_MAX_BASE:
        ...
        legin = df.iloc[j]
        if not is_leg(legin, WEEKLY_LEG_BODY_PCT):
            i -= 1
            continue
        ...
        legin_ok = (
            candle_range(legin) >= zone_range * WEEKLY_LEG_IN_SINGLE_M
            or continuous_leg_ok(
                df, j, -1,
                WEEKLY_CONT_MIN, WEEKLY_CONT_MAX,
                WEEKLY_CONT_BODY_PCT,
                zone_range,
                WEEKLY_LEG_IN_CONT_M
            )
        )
        ...
        legout_ok = (
            candle_range(legout) >= zone_range * WEEKLY_LEG_OUT_SINGLE_M
            or continuous_leg_ok(
                df, i, +1,
                WEEKLY_CONT_MIN, WEEKLY_CONT_MAX,
                WEEKLY_CONT_BODY_PCT,
                zone_range,
                WEEKLY_LEG_OUT_CONT_M
            )
        )
...
def run():
    ...
    quarterly_csv = max(glob.glob(f"{QUARTERLY_CSV_DIR}/*.csv"), key=os.path.getmtime)

    mdf = pd.read_csv(monthly_csv)
    qdf = pd.read_csv(quarterly_csv)

    symbols = sorted(set(mdf["symbol"]).union(set(qdf["symbol"])))
    out = []
    ...
    for sym in symbols:
        ...
        cmp = df["close"].iloc[-1]
        wdf_data = to_weekly(df)
        weekly_zones = scan_zones(wdf_data)

        m_zones = mdf[mdf["symbol"] == sym].to_dict("records")
        q_zones = qdf[qdf["symbol"] == sym].to_dict("records")

        for dz in weekly_zones:

            # CMP distance filter
            if not near_cmp(dz["proximal"], cmp):
                continue

            # 🔒 FINAL PRICE SIDE CHECK
            if dz["type"] == "SUPPLY":
            ...
            for qz in q_zones:
                h = abs(qz["proximal"] - qz["distal"])
                if qz["zone_type"] == "DEMAND":
                    if qz["distal"] <= d_check <= qz["proximal"]:
                        tags.append("QUARTERLY")
                    elif qz["proximal"] < d_check <= qz["proximal"] + h * QUARTERLY_AREA_MULTIPLIER:
                        tags.append("QUARTERLY_AREA")
                else:
                    if qz["proximal"] >= d_check >= qz["distal"]:
                        tags.append("QUARTERLY")
                    elif qz["proximal"] > d_check >= qz["proximal"] - h * QUARTERLY_AREA_MULTIPLIER:
                        tags.append("QUARTERLY_AREA")
            ...
            out.append({
                ...
                "zone_type": dz["type"],
                "pattern": dz["pattern"],
                "zone_create_date": wdf_data.index[dz["idx"]].strftime("%Y-%m-%d"),
    ...
    if out:
        fname = f"weekly_zones_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        pd.DataFrame(out).to_csv(os.path.join(OUTPUT_DIR, fname), index=False)
        print("✅ DONE:", fname)
    else:
        print("⚠️ No valid weekly zones")
