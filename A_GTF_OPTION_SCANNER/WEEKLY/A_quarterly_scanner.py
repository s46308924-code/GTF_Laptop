# 
# DEMAND & SUPPLY TRADING – QUARTERLY SCANNER 
...
QUARTERLY_BASE_BODY_PCT = 0.50
QUARTERLY_LEG_BODY_PCT  = 0.50
QUARTERLY_MAX_BASE = 3

QUARTERLY_LEG_IN_SINGLE_M  = 1.5
QUARTERLY_LEG_OUT_SINGLE_M = 2.0

QUARTERLY_CONT_MIN = 2
QUARTERLY_CONT_MAX = 3
QUARTERLY_CONT_BODY_PCT = 0.50
QUARTERLY_LEG_IN_CONT_M  = 1.5
QUARTERLY_LEG_OUT_CONT_M = 2.0
...
def valid_continuous(df, start_idx, direction, zone_range, multiplier):
    ...
    while i >= 0 and candle_color(df.iloc[i]) == color and len(candles) < QUARTERLY_CONT_MAX:
        if body_pct(df.iloc[i]) < QUARTERLY_CONT_BODY_PCT:
            break
        candles.append(df.iloc[i])
        i -= 1

    if len(candles) < QUARTERLY_CONT_MIN:
        return False
...
def scan_quarterly_zones(df):
    ...
    while i >= 3:
        ...
        if not is_leg(legout, QUARTERLY_LEG_BODY_PCT):
            i -= 1
            continue

        base = []
        j = i - 1
        while j >= 0 and is_base(df.iloc[j], QUARTERLY_BASE_BODY_PCT) and len(base) < QUARTERLY_MAX_BASE:
        ...
        legin = df.iloc[j]
        if not is_leg(legin, QUARTERLY_LEG_BODY_PCT):
            i -= 1
            continue
        ...
        legin_ok = (
            candle_range(legin) >= zone_range * QUARTERLY_LEG_IN_SINGLE_M or
            valid_continuous(df, j, lin, zone_range, QUARTERLY_LEG_IN_CONT_M)
        )

        legout_ok = (
            candle_range(legout) >= zone_range * QUARTERLY_LEG_OUT_SINGLE_M or
            valid_continuous(df, i, lout, zone_range, QUARTERLY_LEG_OUT_CONT_M)
        )
...
def run():
    ...
    for sym in symbols:
        ...
        zones = scan_quarterly_zones(qdf)
    ...
    if results:
        fname = f"quarterly_zones_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        pd.DataFrame(results).to_csv(os.path.join(OUTPUT_DIR, fname), index=False)
        print("✅ QUARTERLY CSV CREATED:", fname)
    else:
        print("⚠️ No valid quarterly zones")
